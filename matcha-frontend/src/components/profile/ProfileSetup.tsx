...
export default function ProfileSetup() {
  ...
  const [catPhotoLoading, setCatPhotoLoading] = useState(false);
  ...
  const handlePhotoChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) {
      const files = Array.from(e.target.files).slice(0, USER_MAX_PHOTOS);
  ...
  const handleRandomCatPhoto = async () => {
    setCatPhotoLoading(true);
    setFieldErrors(prev => {
      const { photos, ...rest } = prev;
      return rest;
    });
    ...
    try {
      ...
      const fileError = validateFile(catFile);
      if (fileError) {
        addToast('Cat photo validation failed', 'error', 3000);
        return;
      }

      const currentPhotos = formData.photos;
      if (currentPhotos.length >= USER_MAX_PHOTOS) {
        addToast(`Maximum ${USER_MAX_PHOTOS} photos allowed. Remove a photo first.`, 'error', 3000);
        return;
      }
      ...
      <form className="space-y-6">
        ...
        <div>
          <label className="block text-sm font-medium mb-4">
            Upload Photos (Max {USER_MAX_PHOTOS})
          </label>
          ...
          <div className="flex gap-3">
            <div className="relative flex-1">
              ...
              <label
                ...
                className="flex items-center justify-center gap-2 px-6 py-3 bg-green-500 text-white rounded-md hover:bg-green-600 cursor-pointer transition-colors w-full"
              >
                <svg
                  className="w-5 h-5"
                  fill="none"
            ...
            <button
              type="button"
              onClick={handleRandomCatPhoto}
              disabled={catPhotoLoading || formData.photos.length >= USER_MAX_PHOTOS}
              className="flex items-center justify-center gap-2 px-6 py-3 bg-gradient-to-r from-orange-500 to-pink-500 text-white rounded-md hover:from-orange-600 hover:to-pink-600 disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-md hover:shadow-lg flex-1"
            >
              {catPhotoLoading ? (
                <>
